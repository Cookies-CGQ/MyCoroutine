...
namespace nsCoroutine
{
    ...
    void Scheduler::start()
    {
        ...
        for(size_t i = 0; i < _m_threadCount; i++)
        {
            ...
            _m_threadIds.push_back(_m_threads[i]->GetThreadId());
        }
        if(debug)
        {
    ...
    {
        std::lock_guard<std::mutex> lock(_m_mutex);
        return _m_stopping && _m_tasks.empty() && _m_activeThreadCount == 0 && _m_activeThreadCount == 0;
    }
}